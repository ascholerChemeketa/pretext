...
function doSearch(searchlocation="A") {
  ...
  if(searchterms != "") {
      pageResult = ptx_lunr_idx.query((q) => {
          for(let term of searchterms.split(' ')) {
              q.term(term, { fields: ["title"], boost: 1000 }); //exact title match with 100x weight
              q.term(term, { wildcard: lunr.Query.wildcard.TRAILING, fields: ["title"], boost: 1000 }); //inexact title 50x weight
              q.term(term, { fields: ["ptxindex"], boost: 80 }); //index match with 80x weight
              q.term(term, { wildcard: lunr.Query.wildcard.TRAILING, fields: ["ptxindex"], boost: 80 }); //inexact index 50x weight
              q.term(term, { fields: ["body"], boost: 2 }); //exact body 2x weight
  ...
  // our lists later. Must be done before any resorting
